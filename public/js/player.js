...
socket.on("updatePlayers", (data) => {
  // Extract balls from the data
  const balls = data.map((player) => player.ball);

  // Clear existing ball elements from the DOM
  ballElements.forEach((ball) => ball.remove());
  ballElements.length = 0; // Clear the array

  // Add new balls to the DOM
  balls.forEach(({ x, y }) => {
  ...
  console.log("Updated balls:", balls);
});

console.log("balls for the first time", balls);
...
function resetGame() {
  ...
  // Apply the transformation and store the result back in the balls variable
