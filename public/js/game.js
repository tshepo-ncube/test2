...
socket.on("recieveBalls", (data) => {
  console.log("Rececing balls");
  //for now pretend theres only one session
  var sess = data;
  console.log(data);
  //
  //console.log("session code: ", sess.sessionCode);
  var ballss__ = [data.balls];
  console.log(ballss__);
  //var ballsFromServer = sess.players;
  // console.log(ballsFromServer[0]);
  // ballss__.forEach(({ x, y }) => {
  //   const ball = document.createElement("div");
  //   ball.setAttribute("class", "ball");
  //   ball.style.cssText = `left: ${x}px; top: ${y}px; `;
  ...
  const ball = document.createElement("div");
  ball.setAttribute("class", "ball");
  ball.style.cssText = `left: ${data.ball.x}px; top: ${data.ball.y}px; `;
...
window.addEventListener("mousemove", function (event) {
  ...
  //     transform: rotateY(${theY}deg) rotateX(${-theX}deg)
  //   `;

  const gravity = 2;
  const friction = 0.01; // Coefficients of friction
  accelerationX = gravity * Math.sin((theY / 180) * Math.PI);
  accelerationY = gravity * Math.sin((theX / 180) * Math.PI);
  frictionX = gravity * Math.cos((theY / 180) * Math.PI) * friction;
  frictionY = gravity * Math.cos((theX / 180) * Math.PI) * friction;
  //   if (gameInProgress) {
  //     const mouseDeltaX = -Math.minmax(mouseStartX - event.clientX, 15);
  //     const mouseDeltaY = -Math.minmax(mouseStartY - event.clientY, 15);
...
function resetGame() {
  ...
  socket.on("recieveBalls", (data) => {
    ...
    ballElements[
      index
    ].style.cssText = `left: ${data.ball.x}px; top: ${data.ball.y}px; `;
  });
  ...
  try {
    ...
    if (accelerationX != undefined && accelerationY != undefined) {
      ...
      socket.on("recieveBalls", (data) => {
        ...
        ballsFromServer.forEach(({ x, y }, index) => {
          ballElements[index].style.cssText = `left: ${x}px; top: ${y}px; `;
        });
      });
      // balls.forEach((ball) => {
